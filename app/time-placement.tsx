...
export function TimePlacement({
  ...
  const [notice, setNotice] = useState("按住卡片，拖进时间链");
  ...
  return (
    <section className="placement-section insertion-section" id="placement">
      <div className="section-heading placement-heading insertion-heading">
        ...
        <p>把事件插进已有时间链。位置、前后关系和同期世界会一起留下记忆。</p>
      </div>

      <div className="placement-shell insertion-shell">
        <div className="placement-tabs insertion-tabs" role="tablist" aria-label="事件嵌入时间模块">
        ...
        {panel === "game" ? (
          ...
          <div className="insertion-records" id="insertion-records">
            <div className="insertion-records-head"><div><span>不评分</span><h3>做题记录</h3></div><p>保留落点与时间差，方便回看。</p></div>
            {records.length ? (
