...
function eventMission(event: HistoryEvent): LearningMission {
  ...
  return {
    ...
    heroImage: `/historical-art/hero-${event.id}.webp`,
    role,
...
function figureMission(figure: HistoryFigure): LearningMission {
  ...
  return {
    ...
    heroImage: `/historical-art/hero-${figure.id}.webp`,
    role,
...
export function HistoryApp({ view = "home", initialPlacementItemKey }: { view?: AppView; initialPlacementItemKey?: string }) {
  ...
  function startPlacement(eventId: string) {
    if (view !== "placement") {
      window.location.assign(`/placement?item=${encodeURIComponent(`event:${eventId}`)}`);
      return;
    }
    ...
    <main className={`site-shell view-${view}`}>
      <header className="topbar">
        ...
        <nav className={menuOpen ? "main-nav is-open" : "main-nav"} aria-label="主要导航">
          ...
          <Link className={view === "placement" ? "active" : ""} href="/placement" onClick={() => setMenuOpen(false)}>事件嵌入时间</Link>
          <Link className={view === "journeys" ? "active" : ""} href="/journeys" onClick={() => setMenuOpen(false)}>专题航线</Link>
          <Link className={view === "records" ? "active" : ""} href="/records" onClick={() => setMenuOpen(false)}>我的长卷</Link>
        </nav>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy">
          <div>
            ...
            <p>一条时间轴串起中国历史。收起时看王朝更替，展开后左右滑动，同步查看中国事件、世界事件与关键人物。</p>
          </div>
          <aside className="today-card" aria-label="今日一问与事件嵌入时间入口">
            <div className="today-card-top">
              <span className="today-label">今日一问 × 事件嵌入时间</span>
              <button type="button" className="today-swap" onClick={rotateTodayEvent} aria-label="换一张今日事件">↻ 换一张</button>
            </div>
            <div className="today-card-meta"><span>{todayEvent.track === "china" ? "中国事件" : "世界事件"} · {todayEvent.temporalType === "series" ? "系列事件" : todayEvent.endYear ? "持续时间段" : "时间点"}</span></div>
      ...
      <section className="home-portals" aria-labelledby="home-portals-title">
        <div className="home-portals-heading">
          ...
          <h2 id="home-portals-title">四种方式走进同一条历史长河。</h2>
        ...
        <div className="portal-grid">
          ...
          <Link href="/placement"><span>02</span><strong>事件嵌入时间</strong><p>按住事件卡插进时间链；松手后看时间差、前后事件与同期世界。</p><i>开始嵌入 →</i></Link>
          <Link href="/journeys"><span>03</span><strong>专题航线</strong><p>把散落事件串成问题路径，比较不同历史选择。</p><i>选择航线 →</i></Link>
          <Link href="/records"><span>04</span><strong>我的长卷</strong><p>翻看点亮、收藏、口述与时间记录，不展示分数。</p><i>查看记录 →</i></Link>
        </div>
      </section>
      </>}
                  ...
                  <header className="era-header">
                    <button className="dynasty-toggle" type="button" aria-expanded={isExpanded} aria-controls={`dynasty-panel-${dynasty.id}`} onClick={() => toggleDynasty(dynasty.id)}>
                      ...
                      <span className="dynasty-toggle-side">
                        <span className="era-counts"><span><b>{chinaCount}</b> 中国事件</span><span><b>{worldCount}</b> 世界事件</span><span><b>{allDynastyFigures.length}</b> 关键人物</span></span>
                    ...
                    <div className="era-rail" aria-label={`${dynasty.label}事件与人物，左右滑动浏览`} tabIndex={0}>
                      ...
                      {railItems.map((item) => item.kind === "event" ? (
                        <button type="button" className={`river-card event-card ${item.event.track}`} key={`event-${item.event.id}`} onClick={() => openEvent(item.event)}>
                          <div className="card-topline"><span>{item.event.track === "china" ? "中国事件" : "世界事件"}</span><i>{item.event.category} · {categoryMarks[item.event.category]}</i></div>
  ...
  return (
    <section className="progress-section" id="progress">
      ...
      <div className="time-record-panel">
        <div className="subheading"><span>时间记录</span><strong>只留下你的落点与时间差，不评分。</strong></div>
        {timeRecords.length ? (
          ...
          <div className="empty-time-records"><span>时</span><p>完成一次“事件嵌入时间”，落点和时间差会保存在这里。</p><Link href="/placement">开始嵌入 <i>→</i></Link></div>
        )}
      </div>
