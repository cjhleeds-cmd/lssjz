      ...
      <div className="event-dialog mission-dialog" role="dialog" aria-modal="true" aria-labelledby="mission-title" tabIndex={-1} ref={dialogRef}>
        ...
        <div className={`dialog-hero mission-dialog-hero ${mission.track}`}>
          ...
          <div className="dialog-meta">
            <span>{mission.subjectType === "figure" ? "人物现场" : mission.track === "china" ? "中国事件" : "世界事件"}</span>
        ...
        <div className="dialog-content mission-content">
          {stage === "materials" && (
            <section className="mission-stage materials-stage">
              ...
              <div className="mission-material-status" aria-live="polite"><span>选择一份材料作为依据</span><strong>{selectedMaterialIds.length ? "已选择" : "请选择"}</strong></div>
                ...
                {mission.materials.map((material) => {
                  ...
                  return (
                    <button type="button" className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => toggleMaterial(material.id)} key={material.id}>
                      ...
                      {selected && <div className="mission-material-clue"><small>口述可引用</small><p>{material.summary.replace("阅读时留意：", "")}</p></div>}
              ...
              <div className="mission-stage-actions">
                <button type="button" className="primary-action" disabled={selectedMaterialIds.length < 1} onClick={() => goToStage("oral")}>
                  {selectedMaterialIds.length ? "带着材料去口述" : "先选 1 份材料"} <span>→</span>
                </button>
              </div>
            </section>
          )}
          ...
          {stage === "oral" && selectedMaterials.length > 0 && (
            <section className="mission-stage oral-stage">
              <button type="button" className="mission-back-button" onClick={goBack}><span>←</span> 返回材料</button>
              <h3>用自己的话讲清这件事</h3>
              ...
              {assessment && (
                <div className={`assessment mission-assessment ${assessment.level}`} aria-live="polite">
                  ...
                  {assessment.level === "followup" && assessment.followUp && <blockquote>{assessment.followUp}</blockquote>}
                  {assessment.level === "support" && <ul>{mission.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul>}
                  <div className="assessment-actions">
                    {assessment.level === "followup" && <button type="button" onClick={() => setAssessment(null)}>继续补充</button>}
                    {assessment.level === "support" && <><button type="button" onClick={() => goToStage("materials")}>回看材料</button><button type="button" onClick={() => setAssessment(null)}>重新组织</button></>}
                    {assessment.level === "pass" && <button type="button" onClick={finishUnlock}>解锁这个事件 <span>→</span></button>}
                  </div>
                </div>
              )}
            </section>
          ...
          {stage === "unlock" && (
            <section className="mission-stage unlock-stage">
